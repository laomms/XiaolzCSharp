...
namespace XiaolzCSharp
{
	public class Main
	{
		...
		public static int RecvicetPrivateMessage(ref PrivateMessageEvent sMsg)
		{
			...
			if (sMsg.MessageType == PInvoke.MessageTypeEnum.FriendUsualMessage)
			{
				if (sMsg.SenderQQ != sMsg.ThisQQ)
				{
					...
					#region 处理好友事件
					else if (sMsg.MessageContent.Contains("同意") && sMsg.MessageContent.Contains("入群") && sMsg.SenderQQ == long.Parse(PInvoke.MasterQQ))
					{
						...
						if (m.Success)
						{
							try
							{
								API.GroupVerificationEvent(PInvoke.plugin_key, sMsg.ThisQQ, API.EventDics[long.Parse(m.Value)].Item1, long.Parse(m.Value), API.EventDics[long.Parse(m.Value)].Item3, PInvoke.GroupVerificationOperateEnum.Agree, PInvoke.EventTypeEnum.Friend_FriendRequest, "同意入群");
								API.EventDics.Remove(long.Parse(m.Value));
								API.SendPrivateMsg(PInvoke.plugin_key, sMsg.ThisQQ, sMsg.SenderQQ, "已处理完毕.", ref MessageRandom, ref MessageReq);
							}
							catch
							{
					...
					else if (sMsg.MessageContent.Contains("拒绝") && sMsg.MessageContent.Contains("入群") && sMsg.SenderQQ == long.Parse(PInvoke.MasterQQ))
					{
						string output = Regex.Replace(sMsg.MessageContent, "[\\d]", string.Empty);
						...
						if (m.Success)
						{
							try
							{
								API.GroupVerificationEvent(PInvoke.plugin_key, sMsg.ThisQQ, API.EventDics[long.Parse(m.Value)].Item1, long.Parse(m.Value), API.EventDics[long.Parse(m.Value)].Item3, PInvoke.GroupVerificationOperateEnum.Deny, PInvoke.EventTypeEnum.Friend_FriendRequest, "拒绝入群");
								API.EventDics.Remove(long.Parse(m.Value));
								API.SendPrivateMsg(PInvoke.plugin_key, sMsg.ThisQQ, sMsg.SenderQQ, "已处理完毕.", ref MessageRandom, ref MessageReq);
							}
							catch
					...
					else if (sMsg.MessageContent.Contains("同意进群") && sMsg.SenderQQ == long.Parse(PInvoke.MasterQQ))
					{
						string output = Regex.Replace(sMsg.MessageContent, "[\\d]", string.Empty);
						...
						if (m.Success)
						{
							try
							{
								var ret = API.GroupVerificationEvent(PInvoke.plugin_key, sMsg.ThisQQ, API.EventDics[long.Parse(m.Value)].Item1, long.Parse(m.Value), API.EventDics[long.Parse(m.Value)].Item3, PInvoke.GroupVerificationOperateEnum.Agree, PInvoke.EventTypeEnum.Group_Invited, "同意入群");
								API.EventDics.Remove(long.Parse(m.Value));
								API.SendPrivateMsg(PInvoke.plugin_key, sMsg.ThisQQ, sMsg.SenderQQ, "已加入群:" + m.Value, ref MessageRandom, ref MessageReq);
							}
							catch
							{
					...
					else if (sMsg.MessageContent.Contains("拒绝进群") && sMsg.SenderQQ == long.Parse(PInvoke.MasterQQ))
					{
						string output = Regex.Replace(sMsg.MessageContent, "[\\d]", string.Empty);
						...
						if (m.Success)
						{
							try
							{
								API.GroupVerificationEvent(PInvoke.plugin_key, sMsg.ThisQQ, API.EventDics[long.Parse(m.Value)].Item1, long.Parse(m.Value), API.EventDics[long.Parse(m.Value)].Item3, PInvoke.GroupVerificationOperateEnum.Agree, PInvoke.EventTypeEnum.Group_MemberInvited, "同意入群");
								API.EventDics.Remove(long.Parse(m.Value));
								API.SendPrivateMsg(PInvoke.plugin_key, sMsg.ThisQQ, sMsg.SenderQQ, "已拒绝邀请.", ref MessageRandom, ref MessageReq);
							}
							catch
							{
					...
					else if (sMsg.MessageContent.Contains("同意加") && sMsg.MessageContent.Contains("为好友") && sMsg.SenderQQ == long.Parse(PInvoke.MasterQQ))
					{
						string output = Regex.Replace(sMsg.MessageContent, "[\\d]", string.Empty);
						...
						if (m.Success)
						{
							try
							{
								API.FriendverificationEvent(PInvoke.plugin_key, sMsg.ThisQQ, long.Parse(m.Value), API.EventDics[long.Parse(m.Value)].Item3, PInvoke.FriendVerificationOperateEnum.Agree);
								API.EventDics.Remove(long.Parse(m.Value));
								API.SendPrivateMsg(PInvoke.plugin_key, sMsg.ThisQQ, sMsg.SenderQQ, "已处理完毕.", ref MessageRandom, ref MessageReq);
							}
							catch
					...
					else if (sMsg.MessageContent.Contains("拒绝加") && sMsg.MessageContent.Contains("为好友") && sMsg.SenderQQ == long.Parse(PInvoke.MasterQQ))
					{
						string output = Regex.Replace(sMsg.MessageContent, "[\\d-]", string.Empty);
						...
						if (m.Success)
						{
							try
							{
								API.FriendverificationEvent(PInvoke.plugin_key, sMsg.ThisQQ, long.Parse(m.Value), API.EventDics[long.Parse(m.Value)].Item3, PInvoke.FriendVerificationOperateEnum.Deny);
			...
			else if (sMsg.MessageType == PInvoke.MessageTypeEnum.Temporary)
			{
                API.SendPrivateMsg(PInvoke.plugin_key, sMsg.ThisQQ, sMsg.SenderQQ, sMsg.SenderQQ.ToString() + "临时消息不回复,请加我位好友或者联系联系网中行(37476230).", ref MessageRandom, ref MessageReq);
            }
                return 0;
		}
		#endregion
		...
		public static int RecvicetGroupMessage(ref GroupMessageEvent sMsg)
		{
			...
			if (sMsg.MessageType == PInvoke.MessageTypeEnum.GroupUsualMessage)
			{
                if (sMsg.SenderQQ != sMsg.ThisQQ)
                {
                    if (sMsg.MessageContent.Contains("发图片"))
            ...
            else if (sMsg.MessageType == PInvoke.MessageTypeEnum.GroupTemporary)
            {
                API.SendGroupMsg(PInvoke.plugin_key, sMsg.ThisQQ, sMsg.MessageGroupQQ, "[@" + sMsg.SenderQQ.ToString() + "]" + "临时消息不回复,请加我位好友或者联系网中行(37476230)." , false);
            }
